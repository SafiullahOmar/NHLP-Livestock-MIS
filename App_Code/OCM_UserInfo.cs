...
WHERE     (aspnet_Users.UserId = CAST('" + userID + "' AS uniqueIdentifier)) and aspnet_Applications.ApplicationName=N'LIVE'");
...
FROM         aspnet_Users INNER JOIN
                      ...
                      aspnet_Applications ON aspnet_Users.ApplicationId = aspnet_Applications.ApplicationId
WHERE     (aspnet_Applications.ApplicationName = N'LIVE') AND (aspnet_Users.UserId = '" + UserID + "')");
            if (dt.Rows.Count > 0)
            {
