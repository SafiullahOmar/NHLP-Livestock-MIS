...
public partial class BenefImport : System.Web.UI.Page
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        if (FileUpload1.HasFile)
        {
            try
            {
                ...
                DataRow[] dtRows = dt.Select("Name is null or FName is  null or  Gender is  null or Mob is null or Position is null");
                ...
                else
                {
                    ...
                    foreach (DataRow rw in dt.Rows)
                    {
                        ...
                        if (rw["Gender"].ToString().ToLower() == "male")
                            Gender = 1;
                        else
                            Gender = 2;
                        ...
                        p[7] = new SqlParameter("@Village", SqlDbType.NVarChar) { Value =hdnVillage.Value};
                        p[8] = new SqlParameter("@PositionId", SqlDbType.NVarChar) { Value = rw["Position"] };
                        p[9] = new SqlParameter("@VFUId", SqlDbType.NVarChar) { Value =ddlVFU.SelectedValue };
                        p[10] = new SqlParameter("@Year", SqlDbType.NVarChar) { Value = ddlYear.SelectedValue };
                        p[11] = new SqlParameter("@Remarks", SqlDbType.NVarChar) { Value = rw["Remarks"] };
                        p[12] = new SqlParameter("@exist", SqlDbType.Bit);
                        p[12].Direction = ParameterDirection.Output;
                        flag = Convert.ToBoolean(db.ExecuteTransStoreProcedureReturn("spPageMembers_Save", p, true, "@exist"));
                        if (flag == false)
                        {
                            count++;
                        }
                    ...
                    db.EndTransaction();
                    lblMsg.Text = count.ToString() + " Members Information has been Added into system.";
    ...
    protected void ddlDistrict_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        com.Parameters.AddWithValue("@DistrictId", SqlDbType.Int).Value =ddlDistrict.SelectedValue;
        con.Open();
        SqlDataReader dr = com.ExecuteReader();
    ...
    protected override void Render(HtmlTextWriter writer)
    {
        ...
        foreach (ListItem option in ddlVillage.Items)
        {
            Page.ClientScript.RegisterForEventValidation(ddlVillage.UniqueID, option.Value);
        }
